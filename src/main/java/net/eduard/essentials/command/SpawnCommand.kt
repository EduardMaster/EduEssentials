    ...
    override fun playerCommand(player: Player, args: Array<String>) {
        if (EduEssentials.getInstance().storage.contains("spawn")) {
            if (EduEssentials.getInstance().configs.getBoolean("spawn.delay.enabled")) {
                if (player.hasPermission(EduEssentials.getInstance().configs
                        .getString("spawn.delay.bypass-permission"))){
                    teleport(player)
                    return
                }
                ...
                player.sendMessage(EduEssentials.getInstance().message("Spawn delay teleport")
                    .replace("%time", "" + delay))
            } else {
                teleport(player)
            }
